...
after `just ops setup` has installed `rclone`, and paste the result into `/infra/csi-rclone`.
...
This is the artifact the whole page builds:
...
2. **Enable SSH support and External Reachability** in the box's settings. Port 22 is always
   ...
   uses. External Reachability is not optional here — the cluster is not inside Hetzner's network.
   Both settings take a few minutes to apply.
...
8. **Replace `key_file` with `key_pem`.** The line rclone just wrote is a filesystem path, and the
   CSI driver mounts the config Secret and nothing else — that path does not exist inside the
   driver container, so every mount fails at authentication. Inline the key instead:
...
Two options you may need and should not set pre-emptively: port 23 gives a restricted shell, and
...
the numbering shifts between rclone versions. Leave `service_account_file` blank. Say **yes** to
...
token back. Either way Google shows an unverified-app warning — expected for a personal client,
and you continue past it.
...
That id is `root_folder_id`. Set it with `rclone config` — edit the `gdrive` remote, advanced
config, `root_folder_id` — or by hand in the file. Every path is then relative to that folder,
...
one and its data is ciphertext forever, and Infisical is not a backup of itself.
...
Before the config leaves the operator machine, both crypts should resolve — an empty listing is a
