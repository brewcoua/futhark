...
- `.netbirdIp` non-empty — the interface is actually addressed
...
That yields one of three states. A node with no peers at all is `ok`, not `degraded` — there is
nothing for it to hold a tunnel to.
...
- `netbird_watchdog_reboot` is true. It defaults to true, because the nodes this exists for are
  ...
  cap the ladder at re-up — worth doing while first observing a node.
...
`/etc/futhark/mesh-watchdog.env` at mode 0600. A node with no field still runs the ladder; it
...
textfile collector — the hostPath mount in
...
mesh, so a hard failure takes this path down with it. What they add is the shape of a failure,
and notice of the repairs that worked — which otherwise leave no trace anywhere.

`futhark_mesh_repairs_total` is the one to watch over time. A node repairing itself is the system
working; a node repairing itself every day has an underlying fault the watchdog is papering over.
...
systemctl mask netbird; systemctl stop netbird   # the drop-in cannot act; the ladder climbs
journalctl -fu futhark-mesh-watchdog
systemctl unmask netbird                  # before the reboot rung's window
```

Set `netbird_watchdog_reboot: false` for that node first. Do it on a worker before a controller.
