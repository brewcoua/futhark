...
from the backups bucket. No object lock, and no default server-side encryption: the backend
supplies SSE-C on each request instead, per [State is remote, and why](#state-is-remote-and-why).
...
| `backblaze-tofu-state` | the backend's key id and key, and the SSE-C key |
